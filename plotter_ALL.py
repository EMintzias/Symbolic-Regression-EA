...
with open(hc_filename, 'rb') as file:
    # Use pickle.load() to load the data from the file.
    hc_data = pickle.load(file)
...
rs_x_arr = np.full((len(rs_data),len(rs_data[0][1])), None, dtype=float)
rs_y_arr = np.full((len(rs_data),len(rs_data[0][1])), None, dtype=float)

for i in range(len(rs_data)):
    for j in range(len(rs_data[0][1])):
        rs_x_arr[i][j] = j
        rs_y_arr[i][j] = rs_data[i][1][j][1]

rs_x_mean = np.mean(np.array(rs_x_arr), axis=0)
rs_y_mean = np.mean(np.array(rs_y_arr),axis=0)

rs_errors = np.std(np.array(rs_y_arr), axis=0) / np.sqrt(np.array(rs_y_arr).shape[0])
rs_x_err = []
rs_y_err = []
rs_err = []
for i in range(len(rs_data[0][1])):
    if (i % 25000 == 0 and i != 0) or i == 99999:
        print(i)
        rs_x_err.append(i)
        rs_y_err.append(rs_y_mean[i])
        rs_err.append(rs_errors[i])
print(len(rs_err))
...
hc_x_arr = np.full((len(hc_data), x_largest), None, dtype=float)
hc_y_arr = np.full((len(hc_data), x_largest), None, dtype=float)

for i in range(len(hc_data)):
    x_start = 0
    y_min = 1e4
    x_max = len(hc_data[i][1])-1
    for j in range(x_largest):
        hc_x_arr[i][j] = j
        if j == hc_data[i][1][x_start][0]:
            y_min = hc_data[i][1][x_start][1]
            if x_start < x_max:
                x_start += 1
        hc_y_arr[i][j] = y_min

hc_x_mean = np.mean(np.array(hc_x_arr), axis=0)
hc_y_mean = np.mean(np.array(hc_y_arr),axis=0)

hc_errors = np.std(np.array(hc_y_arr), axis=0) / np.sqrt(np.array(hc_y_arr).shape[0])
hc_x_err = []
hc_y_err = []
hc_err = []
for i in range(len(hc_x_arr[0])):
    if i % 25000 == 0 and i != 0:
        print(i)
        hc_x_err.append(i)
        hc_y_err.append(hc_y_mean[i])
        hc_err.append(hc_errors[i])
print(len(hc_err))


#%%
# GP DATA
gp_x_arr = np.full((len(gp_data), gp_data[0][0][-1]), None, dtype=float)
gp_y_arr = np.full((len(gp_data), gp_data[0][0][-1]), None, dtype=float)

for i in range(len(gp_data)):
    x_start = 0
    y_min = 1e4
    x_max = len(gp_data[i][1])-1
    for j in range(gp_data[0][0][-1]):
        gp_x_arr[i][j] = j
        if j == gp_data[i][0][x_start]:
            y_min = gp_data[i][1][x_start].MSE
            if x_start < x_max:
                x_start += 1
        gp_y_arr[i][j] = y_min
    print(gp_y_arr[i][-1])
...
# MEANS
gp_x_mean = np.mean(np.array(gp_x_arr), axis=0)
gp_y_mean = np.mean(np.array(gp_y_arr),axis=0)

gp_errors = np.std(np.array(gp_y_arr), axis=0) / np.sqrt(np.array(gp_y_arr).shape[0])
gp_x_err = []
gp_y_err = []
gp_err = []
for i in range(gp_data[0][0][-1]):
    if (i % 25000 == 0 and i != 0) or i == 99999:
        print(i)
        gp_x_err.append(i)
        gp_y_err.append(gp_y_mean[i])
        gp_err.append(gp_errors[i])
print(len(gp_err))
...
plt.plot(rs_x_mean, rs_y_mean, '-', label='RS', color='#3CB371')
...
plt.legend()
plt.grid(True)
plt.savefig('{}/{}_Learning_Curve.pdf'.format(folder, level), dpi=300)
plt.show()
