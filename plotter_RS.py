...
plt.title("Random Search Learning Curve for '{}' (tests: {})".format(folder, len(data)))
plt.xlabel('Evaluations')
plt.ylabel('MSE')
plt.yscale('log')
plt.legend()
plt.grid(True)
plt.savefig('{}/RS_Learning_Curve_{}evals.pdf'.format(folder, len(x_mean)), dpi=300)
plt.show()
...
for j in range(len(data)):
    ...
    fig.savefig('{}/RS_Solution_test#{}.pdf'.format(folder_path, j), dpi=300)
    plt.show()
...
# %%
#######################################################
# Run quick HC on a nice RS solution
